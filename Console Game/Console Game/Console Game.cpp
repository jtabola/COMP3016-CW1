...
public:
    ...
    Player(int startX, int startY, int startHealth)
        : x(startX), y(startY){
        playerHealth = new int(startHealth);
    }

    //Destructor to free the allocated memory when the program ends
    ~Player() {
        delete playerHealth;
    }
    ...
    if (file.is_open()) {
        ...
        while (std::getline(file, line)) {
            if (line.find("currency") != std::string::npos) {
                ...
                maxHealth = std::stoi(line.substr(line.find(":") + 2));
                player.setHealth(maxHealth);
            }
